...
const Categories = ({
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="hidden md:flex font-semibold font-general text-[15px] text-primary">
        Catégories
      </p>
      ...
      <div className="block md:hidden w-full">
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="0">
              Tous les plats{" "}
              <span className="text-gray-400 ml-1">({totalAll})</span>
            </SelectItem>
            {/* Les pizzas */}
            <SelectItem value={firstTagId.toString()}>
              {getCategory(firstTagId)?.category.name}{" "}
              <span className="text-gray-400 ml-1">
                ({getCategory(firstTagId)?.total})
              </span>
            </SelectItem>
            {filteredCategories
              .filter((x) => x.categories.id !== firstTagId)
              .map((cat) => (
                <SelectItem
                  key={cat.categories.id}
                  value={cat.categories.id.toString()}
                >
                  {cat.categories.name}{" "}
      ...
      <div className="hidden md:flex flex-col max-h-[80vh] overflow-y-auto gap-1">
        ...
        >
          ...
          <p
            className={`text-[12px] ${activeTab === 0 ? "text-black" : "text-gray-400"}`}
          >
            {totalAll}
          </p>
        </div>
        ...
        >
          ...
          <p
            className={`text-[12px] ${activeTab === firstTagId ? "text-black" : "text-gray-400"}`}
          >
            {getCategory(firstTagId)?.total}
          </p>
        </div>
        ...
        {filteredCategories
          ...
          .map((cat) => (
            ...
            >
              <p className="font-medium first-letter:uppercase lowercase text-[12px]">
                {cat.categories.name}
              </p>
              <p
                className={`text-[12px] ${cat.categories.id === activeTab ? "text-black" : "text-gray-400"}`}
              >
                {cat.total}
              </p>
            </div>
