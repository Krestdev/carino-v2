     ...
     <div className='grid grid-cols-1 md:grid-cols-2 justify-center gap-7 py-20'>
      ...
      <div className='flex flex-col gap-6 items-start justify-start max-w-[600px] w-full'>
          ...
          <div className='flex flex-col gap-4'>
            {
              items.map((item, index) => (
                <div key={index} className={`flex flex-row gap-6 items-center justify-between w-full px-7 py-5 rounded-[20px] ${index % 2 === 0 && item.price !== 0 ? "bg-[#848484]/10" : item.price === 0 ? "bg-green-500/10" : "border border-[#848484]"}`}>
                  <div className='flex flex-row items-center justify-between w-full'>
                    <div className='flex flex-row items-center gap-6'>
                      <img src={item.image ?? "/images/imagePlaceholder.svg"} alt={item.nom} className='w-20 h-20 rounded-[12px] object-cover' />
                      <div className='flex flex-col gap-0'>
                        <p className='text-[18px] font-bold'>{item.nom}</p>
                        <p className={`text-[18px] font-bold ${item.price === 0 ? "font-mono" : ""}`}>{item.price === 0 ? "Gratuit" : XAF.format(item.price)}</p>
                      </div>
                    </div>
                    <div className='flex flex-col gap-6 items-end'>
                      {item.price !== 0 && <Button onClick={(e) => {
                        e.preventDefault();
                        removeFromCart(item.itemId)
                      }}
                        className='bg-red-500 hover:bg-red-500/80 text-white rounded-[6px]'><LuX /></Button>}
                      <EditProductDialog nom={item.nom} qte={item.qte} id={item.id} itemId={item.itemId} optionsCurrent={item.options} image={item.image}>
                        <Button variant={"outline"} className='border-black text-black rounded-[6px]'>{"Modifier"}</Button>
                      </EditProductDialog>
                    </div>
                  </div>
                </div>
        ...
        <div className='flex flex-col gap-3 w-full items-end'>
          <div>
            <p className='text-[24px] font-normal text-end'>Commande: <span className='text-[28px] font-bold'>{XAF.format(totalPrice())}</span></p>
            <p className='text-[24px] font-normal text-end'>Frais de livraison: <span className='text-[28px] font-bold'>{XAF.format(fees)}</span></p>
          </div>
          <p className='text-[24px] font-normal text-end'>TOTAL: <span className='text-[28px] font-bold'>{XAF.format(totalPrice() + fees)}</span></p>
        </div>
      </div>
    </div> : 
